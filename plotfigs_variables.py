...
colunas = read_columns_from_txt('/home/owner/Documents/jetstream_paper/FW_ revised data files/jet.txt')
...
units = [r'$\rmu_{0}$ (m $\rms^{-1}$)',r'$\rmv_{0}$ (m $\rms^{-1}$)',r'$\rmu_{G}$ (m $\rms^{-1}$)',r'$\rmu_{y}$ ($\rms^{-1}$)',r'$\rmv_{y}$ ($\rms^{-1}$)',r'$\rmw_{0}$ (m $\rms^{-1}$)', r'$\mathrm{\omega}$ (hPa s$^{-1}$)',r'$\mathrm{\theta}$$\rm_{0}$ (K)',r'$\mathrm{\theta}$$\rm_{y}$ (K $\rmm^{-1}$)',r'$\mathrm{\theta}$$\rm_{yy}$ (K $\rmm^{-2}$)',r'$\mathrm{\rho}$$\rm_{0}$ (kg $\rmm^{-3}$)','p (hPa)',r'$\mathrm{\Pi}$$\rm_{0}$ ($\rmm^{2}$ $\rms^{-2}$ $\rmK^{-1}$)',r'$\mathrm{\Pi}$$\rm_{y}$ (m $\rms^{-2}$ $\rmK^{-1}$)',r'$\mathrm{\Pi}$$\rm_{yy}$ ($\rms^{-2}$ $\rmK^{-1}$)']
...
#for k in range(0,2):    
    ...
    if k == 8:
        plt.xlim([-1.06e-5,0])
        plt.xticks([-1e-5,-0.8e-5,-0.6e-5,-0.4e-5,-0.2e-5,0])
    if k == 13:
        plt.xlim([-0.0000106,0])
    if k == 14:
        plt.xlim([-1.16e-12,0])
        plt.xticks([-1e-12,-0.8e-12,-0.6e-12,-0.4e-12,-0.2e-12,0])
...
plt.xticks([-2e-6,-1e-6,0,1e-6,2e-6,3e-6,4e-6])
ax.grid('True',color='k')
ax.tick_params('both', length=23, width=2, which='major',top=True, right=True, bottom=True, left=True)
ax.minorticks_on()
ax.tick_params('both', length=15, width=1, which='minor',top=True, right=True, bottom=True, left=True)
